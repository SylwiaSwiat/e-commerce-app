...
const Register = () => {
  ...
  const errNameLength = (
    <p className="err">Your name must have at least 8 characters</p>
  );
  const errNameSpace = (
    <p className="err">Your name can not have empty strings</p>
  );
  const errMatch = <p className="err">Passwords are not the same!</p>;
  const errEmail = <p className="err">Email is not valid</p>;
  const errPassLength = (
    <p className="err">Your password must have at least 8 characters</p>
  );
  const errPass = (
    <p className="err">
      Your password must have at least one uppercase, one lowercase,
  ...
  return (
    <div className="container">
      <form onSubmit={handleSubmit} className="registerForm">
        ...
        <div className="formBox">
          ...
          <label htmlFor="passwordConfirm">
            ...
            />
            {password !== match && password.length !== 0 && match.length !== 0
              ? errMatch
              : null}
          </label>
      ...
      <div className={`success ${success ? "" : "hide"}`}>
        <button onClick={handleCloseSuccess} className="closeSuccess">
          {" "}
