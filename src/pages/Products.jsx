...
const noResult = <div className='noResult'>
   ...
   <img className='noResultImg' src="./images/noResult.png" alt="" /> </div>
...
const handleType=e=>{
  ...
  const  items = products.filter((item) => {
        ...
        else if (item.title.toLowerCase().includes(searchItem.toString().toLowerCase())) {
            if(type==='all'&&status==='all'){
                return item
            }
        else    if(status===`${status}`&&type==='all') {
    ...
    .map((item) => <Item title={item.title} price={item.price} image={item.image} key={item.id} id={item.id} quantity={item.quantity} handleAddProduct={handleAddProduct} item={item} status={item.status}/>)
    const displayProducts = items.slice(pagesVisited, pagesVisited+productsPerPage)
